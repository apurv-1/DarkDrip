...
router.get('/general-stats', async (req, res) => {
  const [users, sales, products] = await Promise.all([User.find(), Order.find(), Product.find()]);
  res.send({
...
router.get('/bestsellers', async (req, res) => {
  ...
  const products = await Product.find({
    _id: {
      $in: bestsellerIds.map((bestseller) => mongoose.Types.ObjectId(bestseller[0])),
    },
  })
    .lean()
    .exec();
  const result = bestsellerIds.map((currentBestseller) => {
    const fullProduct = products.find((product) => String(product._id) === currentBestseller[0]);
