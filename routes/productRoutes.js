...
router.patch('/:productId', authAdmin, setProduct, authEditOrDeleteProduct, (req, res) => {
...
router.delete('/:productId', authAdmin, setProduct, authEditOrDeleteProduct, async (req, res) => {
...
function verifyProductsSyntax(req, res, next) {
  ...
  if (
    products == null ||
    (products.length > 0 &&
      (!products[0].product || typeof products[0].product !== 'string' || !products[0].amount))
  )
